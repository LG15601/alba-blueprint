#!/usr/bin/env node
/**
 * Alba WhatsApp Bridge — Orchestra Intelligence
 *
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.alba', 'whatsapp', 'session'));
...
const MEDIA_CACHE_DIR = path.join(path.dirname(SESSION_DIR), 'media_cache');
mkdirSync(MEDIA_CACHE_DIR, { recursive: true });

// Download and save media to local cache
async function downloadAndSaveMedia(msg) {
  try {
    ...
    const filename = `media_${randomUUID().slice(0, 12)}${ext}`;
    const filepath = path.join(MEDIA_CACHE_DIR, filename);
    
    writeFileSync(filepath, buffer);
    console.log(`[MEDIA] Downloaded ${ext} to ${filepath}`);
    return filepath;
  } catch (error) {
...
// Build reverse LID→phone mapping from session files
...
const LID_TO_PHONE = loadLidMappings();
console.log(`📋 LID mappings loaded: ${Object.keys(LID_TO_PHONE).length} entries`);
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Alba — Orchestra Intelligence', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      console.log(`[DEBUG] msg: fromMe=${msg.key.fromMe}, remoteJid=${msg.key.remoteJid}, hasMessage=${!!msg.message}`);
      ...
      console.log(`[DEBUG] chatId=${chatId}, senderNumber=${senderNumber}, isGroup=${isGroup}, mode=${WHATSAPP_MODE}`);
      ...
      const resolvedNumber = LID_TO_PHONE[senderNumber] || senderNumber;
      const normalizedSender = resolvedNumber.replace(/^\+/, '');
      console.log(`[DEBUG] allowlist check: sender=${senderNumber}, resolved=${resolvedNumber}, normalized=${normalizedSender}`);
      const isAllowed = ALLOWED_USERS.some(u => u.replace(/^\+/, '') === normalizedSender);
      if (!msg.key.fromMe && ALLOWED_USERS.length > 0 && !isAllowed) {
        continue;
      }
      ...
      if (msg.message.conversation) {
        ...
        const localPath = await downloadAndSaveMedia(msg);
        if (localPath) {
        ...
        hasMedia = true;
        mediaType = 'document';
        // Download document to local cache
        const localPath = await downloadAndSaveMedia(msg);
        if (localPath) {
          mediaUrls.push(localPath);
        }
